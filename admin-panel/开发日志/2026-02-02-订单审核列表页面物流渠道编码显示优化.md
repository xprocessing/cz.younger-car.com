...
**关键点：**
- 添加 `LEFT JOIN logistics l ON orr.logistics_type_id = l.type_id COLLATE utf8mb4_unicode_ci` 关联查询
- 使用表别名 `l` 简化SQL语句
- 同时查询 `code` 字段
- 使用 `COLLATE utf8mb4_unicode_ci` 解决排序规则冲突
...
6. **排序规则统一**：使用 `COLLATE utf8mb4_unicode_ci` 解决不同表之间的排序规则冲突
