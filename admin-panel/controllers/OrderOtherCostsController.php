<?php
require_once ADMIN_PANEL_DIR . '/models/OrderOtherCosts.php';
require_once ADMIN_PANEL_DIR . '/helpers/functions.php';

class OrderOtherCostsController {
    private $orderOtherCostsModel;
    
    public function __construct() {
        $this->orderOtherCostsModel = new OrderOtherCosts();
        session_start();
    }
    ...
    public function index() {
        ...
        if ($platformName || $storeName || $costType || $orderId || $startDate || $endDate) {
            $costs = $this->orderOtherCostsModel->searchWithFilters($platformName, $storeName, $costType, $orderId, $startDate, $endDate, $limit, $offset);
            $totalCount = $this->orderOtherCostsModel->getSearchWithFiltersCount($platformName, $storeName, $costType, $orderId, $startDate, $endDate);
        } else {
            $costs = $this->orderOtherCostsModel->getAll($limit, $offset);
            $totalCount = $this->orderOtherCostsModel->getCount();
        }
        
        $totalPages = ceil($totalCount / $limit);
        $platformList = $this->orderOtherCostsModel->getPlatformList();
        $storeList = $this->orderOtherCostsModel->getStoreList();
        $costTypeList = $this->orderOtherCostsModel->getCostTypeList();
        $title = '订单其他费用管理';
        
        include VIEWS_DIR . '/layouts/header.php';
        include VIEWS_DIR . '/order_other_costs/index.php';
    ...
    public function create() {
        ...
        include VIEWS_DIR . '/layouts/header.php';
        include VIEWS_DIR . '/order_other_costs/create.php';
        include VIEWS_DIR . '/layouts/footer.php';
    }
    ...
    public function createPost() {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            redirect(ADMIN_PANEL_URL . '/order_other_costs.php');
        }
        
        // 验证必填字段
        if (empty($_POST['cost_date'])) {
            showError('费用日期不能为空');
            redirect(ADMIN_PANEL_URL . '/order_other_costs.php?action=create');
        }
        ...
        if ($this->orderOtherCostsModel->create($data)) {
            showSuccess('订单其他费用记录创建成功');
        } else {
            showError('订单其他费用记录创建失败');
        }
    ...
    public function edit() {
        ...
        $cost = $this->orderOtherCostsModel->getById($id);
        if (!$cost) {
            showError('订单其他费用记录不存在');
            redirect(ADMIN_PANEL_URL . '/order_other_costs.php');
        }
        ...
        include VIEWS_DIR . '/layouts/header.php';
        include VIEWS_DIR . '/order_other_costs/edit.php';
        include VIEWS_DIR . '/layouts/footer.php';
    }
    ...
    public function editPost() {
        ...
        if (empty($_POST['cost_date'])) {
            showError('费用日期不能为空');
            redirect(ADMIN_PANEL_URL . '/order_other_costs.php?action=edit&id=' . $id);
        }
        ...
        if ($this->orderOtherCostsModel->update($id, $data)) {
            showSuccess('订单其他费用记录更新成功');
        } else {
            showError('订单其他费用记录更新失败');
        }
    ...
    public function delete() {
        ...
        if ($this->orderOtherCostsModel->delete($id)) {
            showSuccess('订单其他费用记录删除成功');
        } else {
            showError('订单其他费用记录删除失败');
        }
    ...
    public function import() {
        ...
        include VIEWS_DIR . '/layouts/header.php';
        include VIEWS_DIR . '/order_other_costs/import.php';
        include VIEWS_DIR . '/layouts/footer.php';
    }
    ...
    public function importPost() {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            showError('无效的请求方式');
            redirect(ADMIN_PANEL_URL . '/order_other_costs.php?action=import');
        }
        ...
        try {
            $result = $this->orderOtherCostsModel->batchInsert($data);
    ...
    public function export() {
        ...
        // 获取所有符合条件的数据
        $costs = $this->orderOtherCostsModel->getAllWithFilters($platformName, $storeName, $costType, $orderId, $startDate, $endDate);
        
        // 设置CSV文件头
        $filename = 'order_other_costs_export_' . date('YmdHis') . '.csv';
    ...
    public function statistics() {
        ...
        // 获取上个月按店铺名称的费用统计
        $lastMonthByStore = $this->orderOtherCostsModel->getLastMonthByStore();
        ...
        include VIEWS_DIR . '/layouts/header.php';
        include VIEWS_DIR . '/order_other_costs/statistics.php';
        include VIEWS_DIR . '/layouts/footer.php';
    }
}