...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="<?php echo ADMIN_PANEL_URL; ?>/shop_costs.php?action=import_post" 
                      enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="excel_file" class="form-label">选择CSV文件</label>
                        <input type="file" class="form-control" id="excel_file" name="excel_file"
                               accept=".csv,.txt" required>
                        <div class="form-text">支持CSV格式文件，最�?0MB，最�?000条记�?/div>
                    </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fa fa-upload"></i> 开始导�?                        </button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form method="POST" action="<?php echo ADMIN_PANEL_URL; ?>/shop_costs.php?action=import_post">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="csv_content" name="csv_content" rows="8"
                                  placeholder="请粘贴CSV格式的内容，第一行为表头，将被跳�? required></textarea>
            ...
            <div class="card-body">
                <div class="alert alert-info">
                    ...
                    <ul class="mb-0">
                        ...
                        <li>记录数量：不超过1000�?/li>
                        <li>第一行为表头，将被跳�?/li>
                        <li>同一平台、店铺、日期的记录将被视为重复，导入时会跳�?/li>
                    </ul>
                </div>
    ...
    <div class="col-md-4">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <h6>CSV示例�?/h6>
                <pre class="bg-light p-2 rounded small"><code>平台名称,店铺名称,日广告花�?费用类型,费用日期,备注
Amazon-FBA,Shop001,299.50,广告费用,2024-01-15,亚马逊广告推�?eBay,Shop002,149.75,广告费用,2024-01-15,eBay促销活动
Shopify,Shop003,89.99,广告费用,2024-01-15,Shopify店铺推广</code></pre>
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <div class="mb-2">
                    <a href="<?php echo ADMIN_PANEL_URL; ?>/shop_costs.php?action=create" class="btn btn-sm btn-outline-info w-100">
                        <i class="fa fa-plus"></i> 手动添加记录
                    </a>
                </div>
                <div class="mb-2">
                    <a href="<?php echo ADMIN_PANEL_URL; ?>/shop_costs.php" class="btn btn-sm btn-outline-secondary w-100">
                        <i class="fa fa-list"></i> 查看所有记�?                    </a>
                </div>
            </div>
        </div>
    </div>
...
function downloadTemplate() {
    const csvContent = `平台名称,店铺名称,日广告花�?费用类型,费用日期,备注
Amazon-FBA,Shop001,299.50,广告费用,2024-01-15,亚马逊广告推�?eBay,Shop002,149.75,广告费用,2024-01-15,eBay促销活动
Shopify,Shop003,89.99,广告费用,2024-01-15,Shopify店铺推广`;
...
document.getElementById('excel_file').addEventListener('change', function(e) {
    ...
    // 检查文件大�?    if (file.size > 10 * 1024 * 1024) {
        alert('文件大小不能超过10MB');
        e.target.value = '';
        return;
    }
    
    // 检查文件类�?    if (!file.name.match(/\.(csv|txt)$/i)) {
        alert('请选择CSV或TXT格式的文�?);
        e.target.value = '';
        return;
    }
    
    // 读取文件头进行简单验�?    const reader = new FileReader();
    reader.onload = function(e) {
        const content = e.target.result;
        const lines = content.split('\n');
        
        if (lines.length < 2) {
            alert('文件内容不完整，请检查文件格�?);
            e.target.value = '';
            return;
        }
        ...
        if (headerColumns !== 6) {
            alert('文件格式不正确，应有6列，实际�? + headerColumns + '�?);
            e.target.value = '';
            return;
        }
    };
