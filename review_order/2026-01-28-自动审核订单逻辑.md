...
获取店铺列表，物流渠道列表备用。 获取所有订单，遍历订单，查询订单可用的物流渠道和仓库，根据库存再次筛选有货的仓库，然后获取商品在中邮和运德仓库的规格，根据规格，订单信息，可用的channel_code，获取两个仓库对应的运费试算数据，并找到最小运费，及对应的type_id和wid
...
4.通过订单的sku，查找在中邮仓库的规格，然后根据规格，订单信息，可用的channel_code，获取运费试算数据（服务器响应时间可能比较长，要等待3-15秒才有数据），并找到totalFee最小运费，及对应的type_id和wid，channel_code   

5.通过订单的sku，查找在运德仓库的规格，然后根据规格，订单信息，可用的channel_code，获取运费试算数据（服务器响应时间可能比较长，要等待3-15秒才有数据），并找到totalFee最小运费，及对应的type_id和wid ，channel_code   
...
完善 batch_review_orde_v1.py 代码，要求每个函数执行都输出结果