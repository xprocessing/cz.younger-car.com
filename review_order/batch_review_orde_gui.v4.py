...
class BatchReviewGUI:
    ...
    def process_orders(self):
        ...
        try:
            ...
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 获取到 {len(orders_list)} 个订单")
            time.sleep(2)  # 避免请求过快
            ...
            #只处理美国和加拿大订单。两种写法都行
            #target_orders = [order for order in orders_list if order.get("wid") == "0" and order.get("receiver_country_code") in ["US", "CA"]]            
            target_orders = [order for order in target_orders if order.get("receiver_country_code") in ["US", "CA"]]

            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 筛选出 {len(target_orders)} 个待处理订单")
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 待处理订单列表：{target_orders}")
    ...
    def process_single_order(self, order, store_list, logistics_list):
        ...
        #判断是否含有公共仓库wid：5833，5832，17814，5844，5845，如果有就用公共仓库2026-02-05
        public_wid = [5833, 5832, 17814, 5844, 5845]
        # 步骤1：提取两者的共同元素（交集），保持 valid_wid 原有顺序
        common_wid = [wid for wid in valid_wids if wid in public_wid]
        # 步骤2：判断交集是否非空，有则更新 valid_wid，无则保留原数据
        if common_wid:
            valid_wids = common_wid
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 有公共仓库有货，更新有货仓库为：{valid_wids}")
