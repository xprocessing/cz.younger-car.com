...
function getOrderInfo(global_order_no) {
    ...
    fetch(url, {
        ...
        .then(data => {
            ...
            let city = data.data.receive_info.city;
           ...
           pre_fee_weight=pre_fee_weight/1000;
            ...
            ///请求运费接口 https://cz.younger-car.com/ems_api.php?postcode=90210&weight=2.5&length=35&width=25&height=15
          
            let url2 = `https://cz.younger-car.com/chayunfei.php?global_order_no=${global_order_no}&postcode=${postal_code}&weight=${pre_fee_weight}&length=${length}&width=${width}&height=${height}&city= ${encodeURIComponent(city)}`;
              console.log(url2);
              //新窗口打开链接
...
//增加初次运行后，每当鼠标滑过元素.special-row ，增加或者更新按钮，鼠标移开时不要删除按钮
// 统一的添加/更新按钮函数
function ensureFreightButton(row) {
    ...
    if (!button) {
        ...
        button.innerText = '查看运费';
        button.setAttribute('data-freight-btn', ''); // 标记用，方便查找
        button.style.marginLeft = '8px'; // 可选：美化一下间距
        
        // 绑定点击事件（使用当前行的 rowid）
        button.onclick = function(e) {
            e.stopPropagation(); // 防止触发行本身的点击事件
            getOrderInfo(global_order_no);
        };
    ...
    // 如果你以后想更新按钮文字或样式，也可以在这里统一处理
    // button.innerText = '查看运费（更新）';
}

// 1. 初始加载时处理所有已存在的 .special-row
document.querySelectorAll('.special-row').forEach(row => {
    ensureFreightButton(row);
});

// 2. 使用事件委托：每次鼠标进入 .special-row 时都确保按钮存在
document.addEventListener('mouseenter', function(e) {
    const row = e.target.closest('.special-row');
    if (row) {
        ensureFreightButton(row);
    }
}, true); // 使用 capture 阶段以确保更早捕获（可选，但更保险）

// 可选：如果你页面会动态添加新的 .special-row，也可以用 MutationObserver 监听
// 但上面的 mouseenter 已经能覆盖绝大多数动态插入的情况（只要鼠标重新滑过）