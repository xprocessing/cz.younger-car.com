...
// 修复语法错误的极简版
!window.hyphenTip && (function() {
  window.hyphenTip = 1;
  
  // 处理符合条件的文本节点
  function handleTextNode(node) {
    if (node.nodeType !== 3 || !/-.+-/.test(node.textContent) || node.parentElement.dataset.h) return;
    const span = document.createElement('span');
    span.dataset.h = 1;
    span.style.cursor = 'help';
    //span.onmouseover = () => alert(node.textContent.trim());
    //鼠标滑过span 执行查询库存，展示库存
