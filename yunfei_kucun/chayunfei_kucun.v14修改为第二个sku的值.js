...
function ensureInventoryButton(tr) {
    ...
    // 查找SKU值
    // 查找SKU值
    console.log('开始查找SKU值...');
    const skuSpans = tr.querySelectorAll('span');
    let skuValue = null;
    let targetSpan = null;
    // 定义匹配SKU规则的正则：必须包含字母 + 必须包含连字符-
    const skuReg = /^(?=.*[a-zA-Z])(?=.*-).+$/;

    // 新增：定义计数器，统计符合条件的sku数量
    let matchCount = 0;
    for (const itemSpan of skuSpans) {
        const trimText = itemSpan.textContent.trim(); // 去除首尾空格，避免空格干扰匹配
        if (skuReg.test(trimText)) {
            skuValue = trimText;
            matchCount++; // 匹配成功，计数器+1
            // 计数器等于2时，就是第二个符合条件的SKU
            if (matchCount === 3) {
                targetSpan = itemSpan;
                skuValue = trimText;
                console.log('第二个符合条件的skuValue:', skuValue);
                break; // 找到目标值，立即终止循环，提升效率
            }
        }
    }
...
const observer = new MutationObserver((mutations) => {
    ...
    if (shouldProcess) {
        ...
        // 处理库存按钮
        document.querySelectorAll('tr[rowid]').forEach(row => {
            ensureFreightButton(row);
        });
    }
});
