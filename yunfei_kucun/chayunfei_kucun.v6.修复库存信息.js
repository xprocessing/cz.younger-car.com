...
// ========== 库存查询功能 ==========
// 格式化展示数据（表格形式）
const formatInventoryData = (dataList, skuValue) => {
    const sortedList = dataList.sort((a, b) => b.average_age - a.average_age);
    
    let content = `SKU【${skuValue}】库存信息（按平均库龄降序）：\n\n`;
    content += "+-----+------------------------+------------+------------+------------+\n";
    content += "| 序号 | 仓库名称                | 库存总量   | 待到货     | 平均库龄   |\n";
    content += "+-----+------------------------+------------+------------+------------+\n";
    
    sortedList.forEach((item, index) => {
        const whName = (item.wh_name || '未知').padEnd(22, ' ').substring(0, 22);
        const total = (item.total || 0).toString().padStart(10, ' ');
        const pending = (item.pending_num || 0).toString().padStart(10, ' ');
        const avgAge = `${item.average_age || 0}天`.padStart(10, ' ');
        
        content += `| ${(index + 1).toString().padStart(3, ' ')} | ${whName} | ${total} | ${pending} | ${avgAge} |\n`;
        content += "+-----+------------------------+------------+------------+------------+\n";
    });
...
// 配置项说明：
// childList: 监听子节点的增减
// subtree: 监听子孙节点（而非仅直接子节点）
// attributes: 监听元素属性变化
// characterData: 监听文本节点/元素文本内容变化
// attributeOldValue: （可选）记录属性变化前的值
// characterDataOldValue: （可选）记录文本变化前的值
// attributeFilter: （可选）指定只监听特定属性，如['class', 'style']
observer.observe(document.body, { 
    childList: true, 
    subtree: true, 
    attributes: true, 
    characterData: true,
    // attributeOldValue: true, // 如需获取属性旧值可开启
    // characterDataOldValue: true, // 如需获取文本旧值可开启
    // attributeFilter: ['class', 'data-id'] // 如需仅监听特定属性可配置
});
