...
function getOrderInfo(global_order_no) {
    ...
    fetch(url, {
        ...
        .then(data => {
            ...
            // 构建运费查询URL并打开新窗口
            const url2 = `https://cz.younger-car.com/chayunfei.php?global_order_no=${global_order_no}&receiver_country_code=${receiver_country_code}&postcode=${postal_code}&weight=${weight_kg}&length=${length}&width=${width}&height=${height}&city=${encodeURIComponent(city)}`;
            console.log(url2);
...
function getFastOrderInfo(global_order_no) {
    // 构建极速运费查询URL并打开新窗口
    const fastUrl = `https://cz.younger-car.com/chayunfei_fast.php?global_order_no=${global_order_no}`;
