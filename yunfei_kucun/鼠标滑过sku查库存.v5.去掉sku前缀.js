...
(function (window, document) {
    ...
    // 极简无确认弹窗函数
    function showToast(msg, duration = 2000, position = 'center') {
        // 移除旧弹窗（避免重复）
  ...
  `;
    ...
    const fetchInventory = debounce(async (skuValue) => {
        try {
            ...
            // 第一步：获取productId
            skuValue =skuValue.split("-").slice(1).join("-");
            ...
            if (result.data && result.data.childrens && result.data.childrens.length > 0) {
                ...
                showToast(`未找到SKU【${skuValue}】的库存数据`, 1000, 'top');
            }

        } catch (error) {
            console.error('库存查询失败:', error);
            //alert(`查询失败：${error.message}\n请检查网络或权限`);
            showToast(`查询失败：${error.message}\n请检查网络或权限`, 1000, 'bottom');
    ...
    const handleTextNode = (node) => {
        ...
        // 匹配「非减号+减号+非减号」且包含字母
        if (node.nodeType !== 3 || !/(?=.*[a-zA-Z])[^-]-[^-]/.test(node.textContent) || node.parentElement.dataset.lxH) return;
