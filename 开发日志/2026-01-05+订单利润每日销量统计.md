...
- `controllers/OrderProfitController.php` - 调用每日销量统计方法，修改日期范围为60天
- `views/order_profit/stats.php` - 添加每日销量统计图表，标题为"最近60天每日销量统计"

#### 功能说明
根据order_profit表的global_purchase_time字段统计最近60天每日的订单销量，并以柱状图形式展示。
...
- 模型方法返回日期和订单数量
- 控制器调用模型方法并传递给视图
- 控制器中日期范围计算为60天（`strtotime('-60 days')`）
